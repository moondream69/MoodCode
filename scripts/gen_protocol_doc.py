#!/usr/bin/env python3
"""Generate WIRE_PROTOCOL.md from pydantic models in mood_code.core.bus."""
...
from mood_code.core.bus.commands import (
    AgentRunCommand,
    AgentRunResult,
...
from mood_code.core.bus.envelope import EventPushEnvelope
from mood_code.core.bus.events import (
...
def generate() -> str:
    ...
    compact_req_example = {
        ...
        "params": {"session_id": session_id, "focus": "保留任务拆解，压缩工具输出"},
    }
    compact_resp_example = {
        "jsonrpc": "2.0",
        "id": "u-6",
        "result": {"summary_tokens": 1840, "saved_tokens": 12400},
    }
    permission_req_example = {
        "jsonrpc": "2.0",
        "id": "u-7",
        "method": "permission.respond",
        "params": {"tool_use_id": "toolu_01", "decision": "allow_once"},
    }
    permission_resp_example = {
        "jsonrpc": "2.0",
        "id": "u-7",
        "result": {"ok": True},
    }
    ...
    sections = [
        ...
        "\n",
        _model_section("SessionCompactCommand", SessionCompactCommand, compact_req_example),
        "\n",
        _model_section("SessionCompactResult", SessionCompactResult, compact_resp_example),
        "\n",
        _model_section("PermissionRespondCommand", PermissionRespondCommand, permission_req_example),
        "\n",
        _model_section("PermissionRespondResult", PermissionRespondResult, permission_resp_example),
        "\n## Server Push\n\n",
        ...
        _model_section("LogLineEvent", LogLineEvent,
            {"type": "log.line", "run_id": run_id, "level": "INFO",
             "source": "mood_code.core.loop", "message": "step 1 started", "ts": ts}),
        "\n## Session Events\n\n",
        ...
        "\n",
        _model_section("ContextCompactedEvent", ContextCompactedEvent,
            {"type": "context.compacted", "session_id": session_id, "run_id": run_id,
             "original_tokens": 24600, "summary_tokens": 1840, "ts": ts}),
        "\n## Permission Events\n\n",
        _model_section("PermissionRequestedEvent", PermissionRequestedEvent,
            {"type": "permission.requested", "run_id": run_id, "tool_use_id": "toolu_01",
             "tool_name": "bash", "params": {"command": "rm -rf build"},
             "param_preview": "rm -rf build", "session_id": session_id, "ts": ts}),
        "\n",
        _model_section("PermissionGrantedEvent", PermissionGrantedEvent,
            {"type": "permission.granted", "run_id": run_id,
             "tool_use_id": "toolu_01", "decision": "allow_once", "ts": ts}),
        "\n",
        _model_section("PermissionDeniedEvent", PermissionDeniedEvent,
            {"type": "permission.denied", "run_id": run_id,
             "tool_use_id": "toolu_01", "decision": "deny_once", "ts": ts}),
        "\n## Subagent Events\n\n",
        _model_section("SubagentStartedEvent", SubagentStartedEvent,
            {"type": "subagent.started", "run_id": "20260516-100030-def456",
             "parent_run_id": run_id, "description": "审查 bus 模块的模型定义", "ts": ts}),
        "\n",
        _model_section("SubagentFinishedEvent", SubagentFinishedEvent,
            {"type": "subagent.finished", "run_id": "20260516-100030-def456",
             "parent_run_id": run_id, "status": "success", "ts": ts}),
        "\n## Skill Events\n\n",
        _model_section("SkillInvokedEvent", SkillInvokedEvent,
            {"type": "skill.invoked", "skill_name": "review",
             "arguments": "src/mood_code/core/bus", "run_id": run_id, "ts": ts}),
        "\n## Error Codes\n\n",
        "| Code | Name | Meaning |\n",
        "|------|------|---------|\n",
