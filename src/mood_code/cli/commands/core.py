...
_PID_FILE = Path.home() / ".mood" / "mood-core.pid"


# 尝试连接 daemon，成功则正常返回，失败则抛出 ConnectionRefusedError/OSError
async def _ping_check(config: MoodConfig) -> None:
...
def cmd_core_status(config: MoodConfig) -> None:
    try:
...
def cmd_core_start(config: MoodConfig) -> None:
    try:
        asyncio.run(_ping_check(config))
    ...
    proc = subprocess.Popen(
        [sys.executable, "-m", "mood_code.core"],
...
def cmd_core_stop(config: MoodConfig) -> None:
    pid = _running_pid()
    if pid is None:
        print("not running")
        return
