...
import mood_code
from mood_code.core.bus.commands import PongResult
from mood_code.core.bus.envelope import JsonRpcError, JsonRpcSuccess
from mood_code.core.config import MoodConfig


# 同步入口：运行 ping 协程，连接失败时打印错误并退出
def cmd_ping(config: MoodConfig) -> None:
    try:
...
async def _ping(config: MoodConfig) -> None:
    t0 = time.monotonic()
    ...
    req = {
        ...
        "id": "cli-1",
        "method": "core.ping",
        "params": {"client": f"cli/{mood_code.__version__}"},
    }
