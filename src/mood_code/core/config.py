...
_DEFAULT_LOG_FILE = "~/.mood/logs/core.log"
_DEFAULT_LOG_FORMAT = "text"
_DEFAULT_CONFIG_PATH = "~/.mood/config.toml"
...
def get_config() -> MoodConfig:
    config = MoodConfig()

    # .env 必须在读取 MOOD_CONFIG 之前加载，以便 .env 中的 MOOD_CONFIG 能影响 TOML 路径
    load_dotenv(".env", override=False)

    # 若显式指定 MOOD_CONFIG，只读该文件；否则按优先级叠加：全局 → 项目本地
    explicit = os.environ.get("MOOD_CONFIG")
    if explicit:
        config_paths = [Path(explicit).expanduser()]
    else:
        config_paths = [
            Path(_DEFAULT_CONFIG_PATH).expanduser(),
            Path(".mood/config.toml"),
        ]
...
# 用 MOOD_* 环境变量覆盖 config 中对应字段（若变量已设置）
def _apply_env(config: MoodConfig) -> None:
    host = os.environ.get("MOOD_HOST")
    if host is not None:
        config.host = host

    port_str = os.environ.get("MOOD_PORT")
    if port_str is not None:
        try:
            config.port = int(port_str)
        except ValueError:
            raise SystemExit(f"Config error: MOOD_PORT must be an integer, got: {port_str!r}")

    log_level = os.environ.get("MOOD_LOG_LEVEL")
    if log_level is not None:
        config.logging.level = log_level

    log_file = os.environ.get("MOOD_LOG_FILE")
    if log_file is not None:
        config.logging.file = log_file

    log_format = os.environ.get("MOOD_LOG_FORMAT")
    if log_format is not None:
        config.logging.format = log_format

    max_steps_str = os.environ.get("MOOD_MAX_STEPS")
    if max_steps_str is not None:
        try:
            val = int(max_steps_str)
            if val <= 0:
                raise SystemExit(
                    "Config error: MOOD_MAX_STEPS must be a positive integer,"
                    f" got: {max_steps_str!r}"
                )
            config.agent.max_steps = val
        except ValueError:
            raise SystemExit(
                f"Config error: MOOD_MAX_STEPS must be an integer, got: {max_steps_str!r}"
            )

    default_model = os.environ.get("MOOD_LLM_DEFAULT_MODEL")
    if default_model is not None:
        config.llm.default_model = default_model

    trace_enabled = os.environ.get("MOOD_TRACE_ENABLED")
    if trace_enabled is not None:
        config.trace.enabled = trace_enabled.lower() not in ("0", "false", "no")

    trace_file = os.environ.get("MOOD_TRACE_FILE")
    if trace_file is not None:
        config.trace.file = trace_file

    trace_payload = os.environ.get("MOOD_TRACE_INCLUDE_LLM_PAYLOAD")
    if trace_payload is not None:
        config.trace.include_llm_payload = trace_payload.lower() not in ("0", "false", "no")

    perm_timeout = os.environ.get("MOOD_PERMISSION_TIMEOUT_S")
    if perm_timeout is not None:
        try:
            perm_timeout_val = float(perm_timeout)
            if perm_timeout_val < 0:
                raise SystemExit(
                    f"Config error: MOOD_PERMISSION_TIMEOUT_S must be >= 0, got: {perm_timeout!r}"
                )
            config.permission.timeout_s = perm_timeout_val
        except ValueError:
            raise SystemExit(
                f"Config error: MOOD_PERMISSION_TIMEOUT_S must be a number, got: {perm_timeout!r}"
            )

    compact_threshold = os.environ.get("MOOD_COMPACT_THRESHOLD")
    if compact_threshold is not None:
        try:
            ...
            raise SystemExit(
                f"Config error: MOOD_COMPACT_THRESHOLD must be a number, got: {compact_threshold!r}"
            )

    compact_tool_limit = os.environ.get("MOOD_COMPACT_TOOL_LIMIT")
