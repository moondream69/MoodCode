...
from mood_code.core.config import MoodConfig, get_config
from mood_code.core.events.bus import EventBus
from mood_code.core.llm.provider import AnthropicProvider
from mood_code.core.logging_setup import setup_logging
from mood_code.core.mcp.server import McpServerManager
from mood_code.core.permissions.manager import PermissionManager
from mood_code.core.permissions.storage import load_policy_file
from mood_code.core.runner import AgentRunner
from mood_code.core.runs import events_file, new_run_id
from mood_code.core.session import SessionManager, SessionStore
from mood_code.core.trace.record import TraceRecord
from mood_code.core.trace.writer import TraceWriter
from mood_code.core.transport.ipc_broadcaster import IpcEventBroadcaster
from mood_code.core.transport.socket_server import SocketServer, get_connection_writer
...
class CoreApp:
    def __init__(self) -> None:
        ...
        self._config: MoodConfig | None = None
    ...
    async def _ping_handler(self, params: dict[str, Any]) -> PongResult:
        ...
        return PongResult(
            server_version=mood_code.__version__,
    ...
    async def _replay_events(
        ...
        if not path.exists():
            for candidate in Path("~/.mood/sessions").expanduser().glob(
    ...
    async def run(self) -> None:
        ...
        policy_file = Path("~/.mood/policy.toml").expanduser()
        ...
        sessions_root = Path("~/.mood/sessions").expanduser()
        ...
        logger.info("mood-core %s listening addr=%s", mood_code.__version__, addr)
